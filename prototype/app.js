...
(function () {
  ...
  var LANG_SUBJ = [
    { v: "it", flag: "🇮🇹", es: "Italiano", en: "Italian" },
    { v: "fr", flag: "🇫🇷", es: "Francés", en: "French" },
    { v: "en", flag: "🇬🇧", es: "Inglés", en: "English" },
    { v: "pt", flag: "🇵🇹", es: "Portugués", en: "Portuguese" }
  ];
  function langSubjName(v) { var o = LANG_SUBJ.filter(function (x) { return x.v === v; })[0]; return o ? (DB.lang === "en" ? o.en : o.es) : ""; }
  ...
  function sheetHtml() {
    ...
    if (S.sheet.kind === "add-subject") {
      ...
      inner = "<h3>" + t("subjects.newTitle") + '</h3><p class="sub">' + t("subjects.newSub") + "</p>" +
        ...
        '<label class="label" style="margin-top:16px">' + (DB.lang === "en" ? "Language subject (optional)" : "¿Es una asignatura de idioma? (opcional)") + '</label>' +
        '<div class="chip-row">' +
          '<button class="chip-sel ' + (!S.sheet.studyLang ? "on" : "") + '" data-action="sheet-studylang" data-v="">' + (DB.lang === "en" ? "No" : "No") + "</button>" +
          LANG_SUBJ.map(function (o) { return '<button class="chip-sel ' + (S.sheet.studyLang === o.v ? "on" : "") + '" data-action="sheet-studylang" data-v="' + o.v + '">' + o.flag + " " + (DB.lang === "en" ? o.en : o.es) + "</button>"; }).join("") +
        "</div>" +
        (S.sheet.studyLang ? '<p class="txt-sm txt-muted" style="margin-top:8px">' + (DB.lang === "en" ? "Zynqora AI and reviews for this subject will be in " + langSubjName(S.sheet.studyLang) + "." : "La IA y los repasos de esta materia saldrán en " + langSubjName(S.sheet.studyLang) + ".") + "</p>" : "") +
